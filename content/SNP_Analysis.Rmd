...
```{bash}
plink2 --bfile ../Example_Data/final --recode vcf id-paste=iid --out array --allow-extra-chr --mind 0.1 --geno 0.05 --max-alleles 2 --min-alleles 2 --chr 1-10
```
...
```{bash}
plink2 --bfile ../Example_Data/final --make-bed  --mind 0.1 --geno 0.05 --max-alleles 2 --min-alleles 2 --chr 1-10 --allow-extra-chr --out array
```
...
```{r}
library(pcadapt)
filename <- read.pcadapt("array.bed", type = "bed")
